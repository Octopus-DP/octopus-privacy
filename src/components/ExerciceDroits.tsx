...
import { Users, CheckCircle, Clock, AlertCircle, Calendar, Mail, FileText, Plus, Edit, History, Search, X } from 'lucide-react';
...
export function ExerciceDroits({ userData, accessToken, entityId }: ExerciceDroitsProps) {
  ...
  // Fonction pour mapper les types français vers les types SQL anglais
  ...
  // Fonction pour mapper les statuts français vers les statuts SQL anglais
  ...
  // Fonction inverse pour affichage
  ...
  const mapDemandeFormToSQL = (formData: any) => {
    return {
      ...
      date_received: formData.dateDemande ? new Date(formData.dateDemande).toISOString() : (formData.date_received || formData.dateReception || new Date().toISOString()),
  ...
  const handleSave = async (formData: any) => {
    try {
      ...
      const newId = editingDemande ? editingDemande.id : `demande_${Date.now()}`;
  ...
  const handleViewHistory = (demande: any) => {
    setHistoryItem(demande);
    setShowHistory(true);
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex-1">
          ...
          <p className="text-gray-600">
            Suivi des demandes d'exercice de droits des personnes concernées (Articles 15-22 RGPD)
          </p>
        </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardTitle className="text-3xl text-blue-600">{enCours}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardTitle className="text-3xl text-orange-600">{enAttente}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardTitle className="text-3xl text-green-600">{traitees}</CardTitle>
          </CardHeader>
        </Card>
      </div>
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700">
              Vous disposez d'un mois pour répondre aux demandes d'exercice de droits, 
              pouvant être prolongé de 2 mois selon la complexité.
            </p>
          </div>
        </div>
      </div>
              ...
              {filteredDemandes.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-500">
                    {searchTerm ? 'Aucune demande trouvée' : 'Aucune demande enregistrée'}
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={demande.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2.5 py-1 text-xs rounded-full ${typeColors[demande.type]}`}>
                          {demande.type}
                        </span>
                      </td>
                        ...
                        <div className={`flex items-center gap-1 ${isOverdue ? 'text-red-600' : 'text-gray-600'}`}>
                          ...
                          <div>
                            ...
                            <div className={`text-xs ${isOverdue ? 'text-red-500' : 'text-gray-500'}`}>
                              {isOverdue ? `Retard: ${Math.abs(joursRestants)}j` : `${joursRestants}j restants`}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Badge variant={statutColors[demande.statut as keyof typeof statutColors] as any}>
                          {demande.statut}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleViewHistory(demande)}
                          >
